...
class CachedData {
  ...
  async GetUserData(id:string) : Promise<UserData> {
    // Profile content is policy-sensitive and must never be durable in browser
    // extension storage. Remove any legacy cache entry, then read the current
    // policy-filtered Supabase projections for this render only.
    await CachedData.storage.remove(CachedData.USERDATA_KEY + id)
    ...
    return {
      account_id: acc_data[0].account_id,
