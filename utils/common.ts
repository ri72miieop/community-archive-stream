...
import type { EntityURL } from '~InterceptorModules/types';
...
export function safeJSONParse(text: string) {
  try {
    ...
    DevLog((e as Error).message, "error");
    return null;
  }
}
...
export function useSignalState<T>(value: T) {
  const [signal, setSignal] = useState(value);

  const updateSignal = (newValue: T) => {
    setSignal(newValue);
  };

  return [signal, updateSignal, signal] as const;
}
...
export function useToggle(defaultValue = false) {
  const [signal, setSignal] = useState(defaultValue);

  const toggle = () => {
    setSignal(!signal);
  };

  return [signal, toggle, signal] as const;
}
