...
const Button = ({
  ...
  const baseStyles =
    "flex items-center justify-center px-4 py-2 rounded-lg font-medium transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
  const variants = {
...
const SignIn = () => {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      const status = await chrome.storage.session.get("companion-login-status")
      if (live) {
        setUser(data.session?.user || null)
        setConnectionStatus(status["companion-login-status"] || "")
      }
    }
    void refresh()
  ...
  const handleOAuthLogin = async () => {
    try {
      ...
      const response = await sendToBackground({ name: "connect-account" })
      if (!response?.ok) setError(response?.error || "Could not start sign-in.")
    } catch (error) {
      setError(error.message)
    } finally {
      setLoading(false)
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Return to Twitter and open the companion.
                </p>
              ...
              <div className="space-y-3 max-w-[380px] mx-auto">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (
                      window.confirm(
                        "Are you sure you want to reset the cache? This will clear all locally stored data."
                      )
                    ) {
                      handleCacheReset()
                    }
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Connect your CA account to this extension. Your website login
                stays separate.
              </p>
            </div>
