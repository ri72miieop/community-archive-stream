import { extractQuotedTweet,
  extractRetweetedTweet,
  extractTweetFullText,
  extractTweetUnion,
  extractTweetUserId,
  extractTweetUserIsProtected } from "~utils/twe_utils";
...
export interface TweetDataMapped {
...
export class TwitterDataMapper {
  static mapAll(tweetData: Tweet): TweetDataMapped[] {
    try {
      const res = [];
      ...
      // Map mentions
      const mentions = this.mapMentions(
      ...
      const quotedTweetData = extractQuotedTweet(tweetData);
      if (quotedTweetData) {
        const {
          account: QT_account,
          profile: QT_profile,
      ...
      const retweetedTweetData = extractRetweetedTweet(tweetData);
      if (retweetedTweetData) {
        ...
        } = this.mapAll(retweetedTweetData)[0];
  ...
  private static addNonDuplicatedTweetEntry(
    ...
    return {
      ...
      ...(!existingEntries.some(
        (entry) =>
          entry.account &&
          JSON.stringify(entry.account) === JSON.stringify(tweetAccount)
      ...
      ...(!existingEntries.some(
        (entry) =>
          entry.profile &&
          JSON.stringify(entry.profile) === JSON.stringify(tweetProfile)
      ...
      ...(tweetMedia &&
        !existingEntries.some(
          (entry) =>
            entry.media &&
            JSON.stringify(entry.media) === JSON.stringify(tweetMedia)
        ) && { media: tweetMedia }),
      ...
      ...(tweetUrls &&
        !existingEntries.some(
          (entry) =>
            entry.urls &&
            JSON.stringify(entry.urls) === JSON.stringify(tweetUrls)
        ) && { urls: tweetUrls }),
      ...
      ...(tweetMentions &&
        !existingEntries.some(
          (entry) =>
            entry.mentions &&
            JSON.stringify(entry.mentions) === JSON.stringify(tweetMentions)
  ...
  private static mapTweet(tweetData: Tweet): Database.InsertTweets {
    const legacy = tweetData.legacy;
    const fullText = extractTweetFullText(tweetData);

    return {
      tweet_id: legacy.id_str,
      account_id: legacy.user_id_str, // TODO REMOVE: THIS IS JUST TO FORCE ERROR|| userData.rest_id,
  ...
  private static mapAccount(tweetData: Tweet): Database.InsertAccount {
    ...
    return {
      ...
      created_at: new Date(
        oldCreatedAt || userData.core.created_at
      ).toISOString(),
  ...
  private static mapMedia(
    media: any[] | undefined,
    tweetId: string
  ): Database.InsertTweetMedia[] | undefined {
  ...
  private static mapUrls(
    urls: any[] | undefined,
    tweetId: string
  ): Database.InsertTweetURLs[] | undefined {
  ...
  private static mapMentions(
    mentions: any[] | undefined,
    tweetId: string
  ): Database.InsertUserMentions[] | undefined {
  ...
  static getUsers(tweetData: Tweet): {extra_info: string, account_id: string, is_protected: boolean}[] {
    const users = [{extra_info: "tweet", account_id: extractTweetUserId(tweetData), is_protected: extractTweetUserIsProtected(tweetData)}];
    const quotedTweetData = extractQuotedTweet(tweetData);
    if (quotedTweetData) {
      users.push({extra_info: "QT", account_id: extractTweetUserId(quotedTweetData), is_protected: extractTweetUserIsProtected(quotedTweetData)});
    }
    const retweetedTweetData = extractRetweetedTweet(tweetData);
    if (retweetedTweetData) {
      users.push({extra_info: "RT", account_id: extractTweetUserId(retweetedTweetData), is_protected: extractTweetUserIsProtected(retweetedTweetData)});
    }
    return users;
  }
}
